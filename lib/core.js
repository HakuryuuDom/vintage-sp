...
module.exports = function SkillPrediction(mod) {
	...
	const ping = Ping(mod),
		abnormality = Abnormies(mod);

	//TODO: refactor this shit, but im not doing it so go buy nextgen :^) -chi
	let sending = false,
		gameId = null,
		templateId = 0,
		skillsCache = null,
	...
	function enable() {
		...
		try {
			addHook("S_DEFEND_SUCCESS", def.getVersion("S_DEFEND_SUCCESS", mod), sDefendSuccessHandler);
			addHook("S_CANNOT_START_SKILL", def.getVersion("S_CANNOT_START_SKILL", mod), sCannotStartSkillHandler);
			addHook("C_CAN_LOCKON_TARGET", def.getVersion("C_CAN_LOCKON_TARGET", mod), cCanLockonTargetHandler);
			addHook("S_CAN_LOCKON_TARGET", def.getVersion("S_CAN_LOCKON_TARGET", mod), sCanLockonTarget);
			addHook("C_PLAYER_LOCATION", def.getVersion("C_PLAYER_LOCATION", mod), state.hookFakeNotFirst, cPlayerLocationHandler);
			addHook("C_NOTIFY_LOCATION_IN_ACTION", def.getVersion("C_NOTIFY_LOCATION_IN_ACTION", mod), notifyLocation.bind(null, "C_NOTIFY_LOCATION_IN_ACTION", def.getVersion("C_NOTIFY_LOCATION_IN_ACTION", mod)));
			addHook("C_NOTIFY_LOCATION_IN_DASH", def.getVersion("C_NOTIFY_LOCATION_IN_DASH", mod), notifyLocation.bind(null, "C_NOTIFY_LOCATION_IN_DASH", def.getVersion("C_NOTIFY_LOCATION_IN_DASH", mod)));
			for (let packet of [
				["C_START_SKILL", def.getVersion("C_START_SKILL", mod)],
				["C_START_TARGETED_SKILL", def.getVersion("C_START_TARGETED_SKILL", mod)],
				["C_START_COMBO_INSTANT_SKILL", def.getVersion("C_START_COMBO_INSTANT_SKILL", mod)],
				["C_START_INSTANCE_SKILL", def.getVersion("C_START_INSTANCE_SKILL", mod)],
				["C_START_INSTANCE_SKILL_EX", def.getVersion("C_START_INSTANCE_SKILL_EX", mod)],
				["C_PRESS_SKILL", def.getVersion("C_PRESS_SKILL", mod)],
				["C_NOTIMELINE_SKILL", def.getVersion("C_NOTIMELINE_SKILL", mod)]
			])
				addHook(packet[0], "raw", state.hookFake, startSkill.bind(null, ...packet));
			addHook("S_GRANT_SKILL", def.getVersion("S_GRANT_SKILL", mod), sGrantSkillHandler);
			addHook("S_INSTANT_DASH", def.getVersion("S_INSTANT_DASH", mod), sInstantDashHandler);
			addHook("S_INSTANT_MOVE", def.getVersion("S_INSTANT_MOVE", mod), sInstantMoveHandler);
			addHook("S_ACTION_END", def.getVersion("S_ACTION_END", mod), sActionEndHandler);
			addHook("C_CANCEL_SKILL", def.getVersion("C_CANCEL_SKILL", mod), cCancelSkillHandler);
			addHook("S_ACTION_STAGE", def.getVersion("S_ACTION_STAGE", mod), sActionStageHandler);
			addHook("S_CREST_MESSAGE", def.getVersion("S_CREST_MESSAGE", mod), state.hookFake, sCrestMessageHandler);
			addHook("S_START_COOLTIME_SKILL", def.getVersion("S_START_COOLTIME_SKILL", mod), cooldownFix);
			addHook("S_DECREASE_COOLTIME_SKILL", def.getVersion("S_DECREASE_COOLTIME_SKILL", mod), cooldownFix);
			addHook("S_SYSTEM_MESSAGE", def.getVersion("S_SYSTEM_MESSAGE", mod), smtCorrection);
		} catch (err) {
	...
	mod.hook("S_LOGIN", def.getVersion("S_LOGIN", mod), event => {
		skillsCache = {};
		blacklistedSkills = {};
		gameId = event.gameId || mod.game.me.gameId;
		templateId = event.templateId || mod.game.me.templateId;
	...
	mod.hook("S_PLAYER_STAT_UPDATE", def.getVersion("S_PLAYER_STAT_UPDATE", mod), event => {
	...
	mod.hook("S_CREST_INFO", def.getVersion("S_CREST_INFO", mod), event => {
		currentGlyphs = {};
		for (let crest of event.crests)
			currentGlyphs[crest.id] = crest.enable;
	});

	mod.hook("S_CREST_APPLY", def.getVersion("S_CREST_APPLY", mod), event => {
	...
	mod.hook("S_PLAYER_CHANGE_STAMINA", def.getVersion("S_PLAYER_CHANGE_STAMINA", mod), event => {
		currentStamina = event.current;
	});

	mod.hook("S_SPAWN_ME", def.getVersion("S_SPAWN_ME", mod), event => {
	...
	mod.hook("S_SKILL_LIST", def.getVersion("S_SKILL_LIST", mod), event => {
		for (let skill of event.skills) {
			let skillId = parseInt(skill.id);
			if (skill.active) {
	...
	mod.hook("S_USER_STATUS", def.getVersion("S_USER_STATUS", mod), event => {
	...
	function hookInventory() {
		if (!inventoryHook) inventoryHook = mod.hook("S_INVEN", def.getVersion("S_INVEN", mod), event => {
	...
	mod.hook("S_PARTY_MEMBER_LIST", def.getVersion("S_PARTY_MEMBER_LIST", mod), (event) => {
		partyMembers = [];
		for (let member of event.members)
			if (!(gameId === member.gameId))
	...
	mod.hook("S_MOUNT_VEHICLE_EX", def.getVersion("S_MOUNT_VEHICLE_EX", mod), event => {
		if (event.target ===gameId) vehicleEx = event.vehicle;
	});

	mod.hook("S_UNMOUNT_VEHICLE_EX", def.getVersion("S_UNMOUNT_VEHICLE_EX", mod), event => {
		if (event.target === gameId) vehicleEx = null;
	});

	mod.hook("S_MOUNT_VEHICLE", def.getVersion("S_MOUNT_VEHICLE", mod), event => {
		if (event.gameId === gameId) {
			mounted = true;
	...
	mod.hook("S_UNMOUNT_VEHICLE", def.getVersion("S_UNMOUNT_VEHICLE", mod), event => {
		if (event.gameId === gameId) {
			mounted = false;
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(info.talents) {
			for(let talentId of info.talents) {
				if(talents.exists(talentId)) {
					const talentInfo = state.talents[talentId][talents.getLevel(talentId)];
					if(talentInfo.abnormal && !abnormality.exists(talentInfo.abnormal)) continue; 
					if (talentInfo.effect == "speed") passivitySpeedBonuses += talentInfo.value;
					if (talentInfo.effect == "chargeSpeed") chargeSpeed += talentInfo.value;
					break;
	...
	mod.hook("S_EACH_SKILL_RESULT", def.getVersion("S_EACH_SKILL_RESULT", mod), event => {
	...
	function cCanLockonTargetHandler(event) {
		...
		if (info) {
			...
			mod.toClient("S_CAN_LOCKON_TARGET", def.getVersion("S_CAN_LOCKON_TARGET", mod), Object.assign({
				success
			}, event));
		}
	}
	...
	function sendActionStage(opts) {
		...
		mod.toClient("S_ACTION_STAGE", def.getVersion("S_ACTION_STAGE", mod), currentAction = {
			gameId: myChar(),
	...
	function grantCharge(skill, info, stage) {
		...
		mod.toClient("S_GRANT_SKILL", def.getVersion("S_GRANT_SKILL", mod), {
	...
	function sendActionEnd(type, distance) {
		...
		mod.toClient("S_ACTION_END", def.getVersion("S_ACTION_END", mod), {
			gameId: myChar(),
			loc: currentLocation.loc,
			w: currentLocation.w,
			templateId,
	...
	function sendCannotStartSkill(skill) {
		...
		mod.toClient("S_CANNOT_START_SKILL", def.getVersion("S_CANNOT_START_SKILL", mod), {skill: tmp}); 
	}

	function sendSystemMessage(smt, data) {
		mod.toClient("S_SYSTEM_MESSAGE", def.getVersion("S_SYSTEM_MESSAGE", mod), {
	...
	//Check gameId(cid)
	function isMe(id) {
		return (gameId === id || vehicleEx && vehicleEx === id);
	}
	...
	function smtCorrection(event) {
		if (mod.parseSystemMessage(event.message).id == "SMT_BATTLE_SKILL_FAIL_LOW_STAMINA") {
			sendSystemMessage(lowStaminaSystemMessage);
			return false;
		}
	}
